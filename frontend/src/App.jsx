                ...
                return (
                  <div style={{ animation: 'fadeIn 0.5s ease-out', width: '100%', height: 'calc(100dvh - 60px)', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ flex: 1, display: 'flex', gap: '20px', overflow: 'hidden' }}>
                      ...
                      {(selectedNode || review) && (
                        <div style={{ width: '340px', backgroundColor: '#121212', borderRadius: '12px', border: '1px solid var(--border-color)', overflowY: 'auto', display: 'flex', flexDirection: 'column', animation: 'fadeIn 0.3s ease-out' }}>
                          <div style={{ padding: '16px', borderBottom: '1px solid var(--border-color)', backgroundColor: '#18181b', position: 'sticky', top: 0, zIndex: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <h3 style={{ margin: 0, fontSize: '1rem', color: '#f4f4f5', display: 'flex', alignItems: 'center', gap: '8px' }}>
                              {selectedNode ? '🎛️ Node Inspector' : '🔍 Architectural Review'}
                            </h3>
                            {selectedNode && (
                              <button onClick={() => setSelectedNode(null)} style={{ background: 'none', border: 'none', color: '#a1a1aa', cursor: 'pointer', fontSize: '18px' }}>&times;</button>
                            )}
                          </div>
                          ...
                          {selectedNode ? (
                            <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                              <div>
                                <h4 style={{ margin: '0 0 4px 0', fontSize: '11px', color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '1px' }}>Service Name</h4>
                                <div style={{ fontSize: '16px', color: '#fff', fontWeight: 'bold' }}>{selectedNode.label}</div>
                              </div>
                              {selectedNode.type && (
                                <div>
                                  <h4 style={{ margin: '0 0 4px 0', fontSize: '11px', color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '1px' }}>Component Type</h4>
                                  <div style={{ display: 'inline-block', background: 'rgba(255,255,255,0.05)', padding: '4px 8px', borderRadius: '4px', color: '#d4d4d8', fontSize: '13px' }}>{selectedNode.type}</div>
                                </div>
                              )}
                              {selectedNode.tech && (
                                <div>
                                  <h4 style={{ margin: '0 0 4px 0', fontSize: '11px', color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '1px' }}>Technology Stack</h4>
                                  <div style={{ display: 'inline-block', background: 'rgba(59,130,246,0.1)', border: '1px solid #3b82f6', padding: '4px 8px', borderRadius: '4px', color: '#3b82f6', fontSize: '13px' }}>{selectedNode.tech}</div>
                                </div>
                              )}
                              {selectedNode.status && (
                                <div>
                                  <h4 style={{ margin: '0 0 4px 0', fontSize: '11px', color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '1px' }}>Health Status</h4>
                                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: selectedNode.status.toLowerCase() === 'healthy' ? 'rgba(16,185,129,0.1)' : 'rgba(245,158,11,0.1)', border: `1px solid ${selectedNode.status.toLowerCase() === 'healthy' ? '#10b981' : '#f59e0b'}`, padding: '4px 8px', borderRadius: '4px', color: selectedNode.status.toLowerCase() === 'healthy' ? '#10b981' : '#f59e0b', fontSize: '13px' }}>
                                    <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: selectedNode.status.toLowerCase() === 'healthy' ? '#10b981' : '#f59e0b' }} />
                                    {selectedNode.status}
                                  </div>
                                </div>
                              )}
                              {selectedNode.description && (
                                <div>
                                  <h4 style={{ margin: '0 0 4px 0', fontSize: '11px', color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '1px' }}>Description</h4>
                                  <p style={{ margin: 0, fontSize: '13px', color: '#d4d4d8', lineHeight: 1.5 }}>{selectedNode.description}</p>
                                </div>
                              )}
                              {selectedNode.zone && (
                                <div>
                                  <h4 style={{ margin: '0 0 4px 0', fontSize: '11px', color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '1px' }}>Architectural Zone</h4>
                                  <div style={{ fontSize: '13px', color: '#9ca3af' }}>{selectedNode.zone.toUpperCase()}</div>
                                </div>
                              )}
                            </div>
                          ) : (
