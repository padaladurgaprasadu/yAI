...
class ArchitectAgent(BaseAgent):
    ...
    def run(self, state: AiONState) -> AiONState:
        ...
        system_prompt = f"You are an Elite Enterprise Systems Architect acting as a {agent_role}. Given a goal and a list of modules, design a highly advanced, cutting-edge technology stack and blueprint. Do NOT build simple 1990s CRUD apps; incorporate modern UX, AI where relevant, scalable structures, and robust data models. Use the provided Past Projects as inspiration if relevant.\n\n{tech_rule}\n\nMULTILINGUAL CAPABILITY: Any notes, descriptions, or comments you generate in the blueprint MUST be written in the same language as the user's Goal.\n\nReturn ONLY valid JSON with four keys: 'tech_stack' (a list of strings), 'designSystem' (a dictionary with style tokens, ONLY if Web/UI project), 'blueprint_notes' (a short string), and 'file_structure' (a comprehensive list of file paths needed for the app). Do not include markdown formatting or backticks, just the raw JSON."
